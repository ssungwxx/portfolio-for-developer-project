        ...
        <div>
            <v-flex xs12 text-xs-center round my-5>
                ...
                <v-btn
                    ...
                    dark
                    @click="deletepost"
                    v-if="getIsLogin && getId == post.user_id"
                >
                    <v-icon size="25" class="mr-2">delete</v-icon>삭제
                </v-btn>
            </v-flex>
        </div>

        <div
            style="display: flex; width: 90%; margin: auto; justify-content: flex-end;"
            v-if="getIsLogin"
            ...
            <table>
                ...
                <tr v-for="(comment, i) of comments" :key="i">
                    ...
                    <td class="post-detail-buttons" v-if="getIsLogin && getId == comment.user_id">
                        ...
                        <v-btn
                            class="post-detail-button"
                            v-if="getIsLogin && getId == comment.user_id"
                            icon
...
export default {
    ...
    methods: {
        ...
        async getPost() {
            this.post = await RestService.getPostDetail(
                this.user_id,
                this.post_index - 1
            );
        ...
        async getComments() {
            const comments = await RestService.getOnePostComments(
                this.post.post_no
            );
            this.comments = comments.data;
            for (let i = 0; i < this.comments.length; ++i) {
                this.comments[i].pcom_date = Git.calendar_time(
                    this.comments[i].pcom_date
                );
            }
        },
        async insertLog() {
