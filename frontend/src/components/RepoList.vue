<template>
    <v-container mt-5>
        <v-layout wrap>
            <v-flex
                class="flexlist"
                v-for="i in repos.length > loadlimits ? loadlimits : repos.length"
                xs8
                style="display: flex"
            >
                <Repo
                ...
                <div class="buttons">
                    ...
                    <v-tooltip bottom>
                        <v-btn
                            slot="activator"
                            icon
                            @click="deleterepo(repos[i - 1].repo_no)"
                            v-if="getIsLogin && getId == user_id"
                        >
            ...
            <div class="plus" v-if="!loadMore">
                ...
                >
                    ...
                    <v-flex style="margin-left: 10px" hidden-xs-only>더 보기</v-flex>
                </v-btn>

                <div style="display: flex; justify-content: center;">
                    <router-link
                        :to="Addrepos"
                        style="text-decoration: none; margin-right: 1.2vw;"
                        v-if="getIsLogin && getId == user_id"
                    >
                        <v-btn
                            class="target"
                            style="margin-right: auto; margin-top: 1vw"
                            color="#ffc0cb"
                            dark
                        >
                            <v-icon size="25">fa-edit</v-icon>
                            <v-flex style="margin-left: 10px" hidden-xs-only>추가하기</v-flex>
                        </v-btn>
                    </router-link>
                    <router-link :to="goback" style="text-decoration: none;">
                        <v-btn
                            ...
                            dark
                        >
                            <v-icon size="25">fa-home</v-icon>
                            <v-flex style="margin-left: 10px" hidden-xs-only>돌아가기</v-flex>
                        </v-btn>
                    </router-link>
                </div>
            </div>
...
.buttons {
    ...
    box-shadow: 1px 2px 0px 0px rgba(0, 0, 0, 0.2),
        1px 1px 0px 0px rgba(0, 0, 0, 0.14),
        3px 1px 0px -1px rgba(0, 0, 0, 0.12);
