      ...
      <v-flex xs8>
        ...
        <h2 class="font-weight-regular">
          <v-btn flat :href="'https://lab.ssafy.com/' + repos.path" target="_blank">
            <v-clamp autoresize :max-lines="1">{{name}}</v-clamp>
          </v-btn>
        </h2>
        <!-- </v-clamp> -->
        <p
          class="subheading mb-1 grey--text text--darken-1 font-weight-light namespace"
        >{{'@' + repos.name}}</p>
      </v-flex>
...
export default {
  ...
  mounted() {
    this.drawStatGraph(), this.getCommits();
  },
  methods: {
    async drawStatGraph() {
      let toD = new Date();
    ...
    async getCommits() {
      let toD = new Date();
      const response = await GitlabService.getCommits(
        this.repos.id,
        this.token,
        toD.toISOString()
      );
      ...
      const responseRepo = await GitlabService.getAllRepos(
        this.repos.name,
        this.token
      );
      ...
      for (const data of responseRepo.data) {
        ...
        const responseCommit = await GitlabService.getRepoCommit(
          this.repos.name,
          repoName,
          this.token
        );
    ...
    async getDateTable() {
      ...
      while (
        ...
        let month =
          (lastDay.getMonth() + 1 + "").length == 1
            ? "0" + (lastDay.getMonth() + 1)
            : lastDay.getMonth() + 1 + "";
        let day =
          (lastDay.getDate() + "").length == 1
            ? "0" + lastDay.getDate()
            : lastDay.getDate() + "";
    ...
    async getChartData() {
      ...
      if (this.commits.length != 0) {
        ...
        for (const d of this.commits) {
          ...
          if (
            d.committer_name == this.cname ||
            d.committer_name == this.name ||
            d.committer_name == this.cname2
          ) {
    ...
    async finishData() {
      ...
      for (const c of Cdata) {
        c.title =
          c.value != 1
            ? c.title + " : " + c.value + "commits"
            : c.title + " : " + c.value + " commit";
      }
      this.data = Cdata;
      return;
    }
